...
The policy is deny-by-pattern on paths plus a scrub pass over content, and it is what bounds the risk
now that blob storage is the default rather than opt-in.

A denied file keeps its identity in the **triple sidecar** -- its ``hasPath`` fact is emitted, so
lineage over the sidecar stays complete -- while its bytes are withheld. Provenance does not require
publication.

Measured caveat, previously overstated here: the denied file does **not** appear in the exported
manifest at all, not even as a bare node. A query asking "did anything touch X" against a manifest
...
class ContentPolicy:
    ...
    def prepare(self, path: str | None, content: bytes | None) -> tuple[bytes | None, bool]:
