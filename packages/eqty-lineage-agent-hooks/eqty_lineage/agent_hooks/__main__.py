...
def _verify(args) -> int:
    ...
    sessions = [
        p for p in find_sessions() if args.min_records <= sum(1 for _ in p.open(errors="replace")) <= args.max_records
    ][: args.limit]
    if not sessions:
    ...
    # Stays off even though recording now defaults to on: `verify` re-records sessions into a
    # throwaway directory only to compare graph *shapes*, and writing every file the corpus ever
    # read to disk to answer a conformance question would be a large and pointless side effect.
...
def _secret(explicit: str | None, env_var: str) -> str | None:
...
def _install(args) -> int:
    ...
    config = {
        "hooks": {
            event: [{"hooks": [session_start if event == "SessionStart" else handler]}] for event in CLAUDE_CODE_EVENTS
        }
    }
...
def _parser() -> argparse.ArgumentParser:
    """The CLI definition, separated so the flags can be asserted on without running a command."""
    ...
    parser.add_argument(
        "--no-blobs",
        action="store_true",
        help="do not store file contents in the blob store; the manifest then carries "
        "identities and derivation edges but not the bytes",
    )
    # Accepted and ignored: blob storage is now the default, and scripts written against the old flag
    # should keep working rather than fail on an unrecognised argument.
    parser.add_argument("--blobs", action="store_true", help=argparse.SUPPRESS)
    ...
    s.add_argument(
        "--token",
        help=f"require this bearer token on every request (prefer ${TOKEN_ENV}, "
    ...
    r.add_argument(
        "--deny", action="append", metavar="NAME=GLOB", help="deny writes matching GLOB under policy NAME (repeatable)"
    )
    r.add_argument(
        "--allow",
        action="append",
    ...
    i.add_argument(
        "--no-token", action="store_true", help=f"omit the ${TOKEN_ENV} Authorization header from the emitted config"
    )
