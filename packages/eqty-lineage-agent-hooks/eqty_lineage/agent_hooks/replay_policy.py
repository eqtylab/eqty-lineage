...
class PolicyOutcome:
    ...
    def summary(self) -> str:
        ...
        return f"{self.policy}: {len(self.denied)} call(s) refused, {len(self.lost)} entities unreachable" + (
    ...
    def contingent(self) -> dict[str, list[str]]:
        ...
        for entity, mask in self.support.items():
            if mask != self.all_policies and mask:
                out[self.paths.get(entity, entity)] = [p for i, p in enumerate(self.policies) if mask >> i & 1]
        return out
