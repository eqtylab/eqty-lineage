...
from .dialects import CLAUDE_CODE, detect_dialect, to_events
...
class HookReceiver:
    ...
    def handle(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.registry.lock_for(session_id):
            ...
            return self._response(event_name, payload, state, session_id, dialect)

    def _response(self, event_name, payload, state, session_id, dialect=CLAUDE_CODE) -> dict[str, Any]:
        if event_name == "SessionStart" and self.watch_paths and dialect == CLAUDE_CODE:
            ...
            #
            # Claude Code only. `watchPaths` does not appear anywhere in the codex-cli 0.148.0 binary,
            # and Codex's hook output wires reject unknown keys -- sending it there risks the whole
            # response being discarded, taking any permission decision in it along too.
