...
``PermissionRequest`` -- so those asymmetries are declared, not demonstrated. Nor can it reach a
subagent's own transcript, which lives in a sibling directory, so the offline path's subagent
inlining is disabled here to keep the comparison like-for-like.
"""
...
def _without_coverage(triples):
    """Drop the coverage statement and the entity it produced.

    Coverage describes the *capture*, not the computation: it says how much each path managed to
    observe, and the two paths observe different things by construction -- the transcript carries the
    model's messages and the hook stream does not. So the coverage nodes differ whenever the paths
    differ, which is always, and comparing them would report a guaranteed divergence that says
    nothing about whether the adapters agree.

    This is the same exclusion `graph_diff` already makes for `session_id` and the `observed` flag,
    and for the same stated reason: they are properties of the capture rather than of the work.
    """
    triples = list(triples)
    coverage_acts = {t.subject for t in triples if t.predicate == prov.LABEL and str(t.object).startswith("coverage:")}
    coverage_entities = {
        t.subject for t in triples if t.predicate == prov.WAS_GENERATED_BY and t.object in coverage_acts
    }
    excluded = coverage_acts | coverage_entities
    return [t for t in triples if t.subject not in excluded and t.object not in excluded]
...
def compare(transcript: Path) -> Report:
    ...
    # Subagent inlining is switched off for the comparison, not because the live path lacks the
    # data -- hooks do fire inside subagents, carrying `agent_id` -- but because `replay_payloads`
    # synthesizes payloads from the main transcript alone and cannot reach a subagent's own file.
    # Leaving it on would make every subagent's tool call an offline-only activity and turn a
    # harness limitation into a reported conformance failure. This is the same class of gap the
    # module already declares for InstructionsLoaded, FileChanged and PermissionRequest.
    offline_events = list(ClaudeCodeTranscript(transcript, include_subagents=False).events())
    ...
    offline = _without_coverage(offline)
    live = _without_coverage(live)
    ...
    inferred: set[str] = {
        str(v.asset_cid) for versions in offline_rec.file_versions.values() for v in versions if not v.observed
    }
