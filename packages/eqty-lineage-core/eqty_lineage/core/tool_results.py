...
from .events import EditAttempt, FileObserved


def apply_edit(original: str | None, old: str | None, new: str | None, replace_all: bool) -> str | None:
...
def file_events_from_result(
...
) -> tuple[list[FileObserved], str | None]:
    ...
    if is_edit:
        ...
        if isinstance(result.get("content"), str):
            ...
            updated: str | None = result["content"]
        else:
            updated = apply_edit(
        ...
        # When the post-image could not be established here, hand the replacement on rather than
        # dropping it. `originalFile` is null on most Edit results, so this is the common case, and
        # the recorder can usually replay it against content the session already knows.
        attempt = None
        if updated is None:
            old, new = result.get("oldString"), result.get("newString")
            if isinstance(old, str) and isinstance(new, str):
                attempt = EditAttempt(old=old, new=new, replace_all=bool(result.get("replaceAll")))
