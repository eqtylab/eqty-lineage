...
def _cids_in(value: Any, found: set[str]) -> None:
    if isinstance(value, str):
        if value.startswith((_CID_PREFIX, "baf", "bag")):
...
def project_manifest(source: Path, dest: Path, keep: set[str]) -> ProjectionStats:
    ...
    for cid, statement in statements.items():
        ...
        if kind == "DataRegistration":
            ...
            if cid in keep:
                kept[cid] = statement
        elif kind == "MetadataRegistration" and statement.get("subject") in keep:
            kept[cid] = statement
    ...
    for cid, statement in statements.items():
        if (
            ...
            and statement.get("@type") == "MetadataRegistration"
            and statement.get("subject") in kept
        ):
            kept[cid] = statement
...
def select_determination(triples: Iterable, activities: Iterable[str]) -> set[str]:
    """CIDs to keep for a determination view: the given activities, their inputs, and their outputs.

    A replication manifest holds every run in one context -- ~1,900 statements for six runs of a small
    task -- and the determination fan-in is a handful of nodes inside it. Rendered whole it is an
    illegible column and the structure the replication exists to show is invisible. This selects that
    structure and nothing else.

    ``activities`` is supplied by the caller rather than inferred. Inferring it from the annotations
    only divergent paths carry -- contested regions -- silently drops every *robust* verdict, which is
    the one thing this view must not lose: robust and divergent side by side is the comparison, and a
    projection showing only the divergent ones would imply nothing agreed.

    Deliberately excluded: the tool calls and model artifacts of the individual runs. "How did this run
    get here" is a question about one run, and ``select_file_lineage`` already answers it.
    """
    triples = list(triples)
    keep: set[str] = {str(a) for a in activities}

    for t in triples:
        if t.predicate == prov.USED and t.subject in keep:
            keep.add(t.object)  # the outcomes an activity compared
        elif t.predicate == prov.WAS_GENERATED_BY and t.object in keep:
            keep.add(t.subject)  # the verdict it produced
    return keep
