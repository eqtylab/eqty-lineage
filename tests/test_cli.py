...
class TestDefaults:
    def test_blob_storage_is_on_by_default(self):
        """A manifest without blobs attests that a file existed and cannot show it.

        What bounds the risk is the redaction policy, not this default: a `.env` the agent reads has
        its bytes withheld from every blob and the omission counted. `--no-blobs` turns it off.
        """
        ...
        assert SessionRegistry().store_blobs is True

    def test_the_old_blobs_flag_is_still_accepted(self):
        # It was the way to turn this on. Scripts carrying it should not start failing on an
        # unrecognised argument now that it names the default.
        from eqty_lineage.agent_hooks.__main__ import _parser

        parsed = _parser().parse_args(["--blobs", "serve"])
        assert parsed.no_blobs is False

    def test_no_blobs_turns_it_off(self):
        from eqty_lineage.agent_hooks.__main__ import _parser

        assert _parser().parse_args(["--no-blobs", "serve"]).no_blobs is True

    def test_store_blobs_reaches_the_sdk(self, monkeypatch):
        """The flag is only worth anything if it arrives at ``set_store_all_blobs``.

        Asserted through a stub rather than a real ``init()``: that call is process-global and raises
        on a second invocation, so a test that flipped it for real would poison every later test in
        the process. Measured end to end on a recorded Codex session, the difference is 16 blobs
        versus 28 -- with it off, the manifest carries the patch's *identity* but not the patch, the
        file's CID but not its bytes, and the prompt's node but not its text.
        """
        seen = {}

        class _Init:
            def set_store_all_blobs(self, value):
                seen["value"] = value
                return self
        ...
        monkeypatch.setattr(eqty_sdk, "init", lambda *a, **k: _Init())
        monkeypatch.setattr(eqty_sdk, "set_active_signer", lambda *a, **k: None)
        monkeypatch.setattr(eqty_sdk, "Signer", type("S", (), {"new": staticmethod(lambda **k: None)}))

        registry = SessionRegistry(store_blobs=True)
        registry._ensure_sdk()
