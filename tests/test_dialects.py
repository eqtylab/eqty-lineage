...
class TestFailures:
    ...
    def test_the_error_text_reaches_the_event(self):
        [ended] = events_of(
            hook("PostToolUseFailure", tool_name="Bash", tool_use_id="t1", error="ENOENT: no such file")
        )
    ...
    def test_an_interrupt_is_distinguishable_from_a_tool_failure(self):
        ...
        [ended] = events_of(
            hook("PostToolUseFailure", tool_name="Bash", tool_use_id="t1", error="stopped", is_interrupt=True)
        )
        assert ended.result == {"error": "stopped", "interrupted": True}

    def test_success_still_reads_tool_response(self):
        [ended] = events_of(hook("PostToolUse", tool_name="Bash", tool_use_id="t1", tool_response="ok"))
