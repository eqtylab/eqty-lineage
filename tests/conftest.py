...
def codex_payloads() -> list:
    ...
    Kept as a recording rather than a hand-written approximation: every Codex-specific finding in this
    suite -- ``apply_patch`` carrying a patch document, lifecycle events lacking ``turn_id`` -- came from
    reading these, not from reading documentation.
    """
...
def pytest_report_header(config):
    try:
        ...
        return None
    backend = "python + rust accelerator" if RUST_AVAILABLE else "python only"
    return f"eqty-lineage backends: {backend} (py{sys.version_info.major}.{sys.version_info.minor})"
